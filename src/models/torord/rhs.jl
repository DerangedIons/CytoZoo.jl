function _torord_rhs_impl!(
    ...
    IKr_Multiplier_spatial =
        T(_resolve_parameter(one(T), overrides, Val(:IKr_Multiplier), x, t))
    ...
    bj = (
        if (v > T(-40.0))
            (
                (T(0.6) * exp(T(0.157) * v)) ./
                (exp(T(0.1) * v) + T(0.0407622039783662))
            )
        else
            (
                (T(0.02424) * exp(T(0.12728) * v)) ./
                (exp(T(0.1378) * v) + T(0.00396086833990426))
            )
        end
    )
    ...
    taum =
        T(0.06487) * exp(-((v - T(4.823)) / T(51.12))^T(2)) +
    ...
    ECl = ((R .* T_val) ./ ((F_param .* zcl))) .* log(clo ./ cli)
    vffrt = (F_param .* (F_param .* v)) ./ ((R .* T_val))
    vfrt = (F_param .* v) ./ ((R .* T_val))
    EClss = ((R .* T_val) ./ ((F_param .* zcl))) .* log(clo ./ clss)
    EK = ((R .* T_val) ./ ((F_param .* zk))) .* log(ko ./ ki)
    Ii = (T(0.5) * (T(4.0) * cai + (cli + (ki + nai)))) / T(1000.0)
    EKs = ((R .* T_val) ./ ((F_param .* zk))) .* log((PKNa .* nao + ko) ./ (PKNa .* nai + ki))
    ENa = ((R .* T_val) ./ ((F_param .* zna))) .* log(nao ./ nai)
    ...
    K1ss = aK1 ./ (aK1 + bK1)
    dclss_dt = -JdiffCl + (Acap .* IClCa_junc) ./ ((F_param .* vss))
    du[21] = dclss_dt
    IClCa = IClCa_junc + IClCa_sl
    dcli_dt = (Acap .* (IClCa_sl + IClb)) ./ ((F_param .* vmyo)) + (JdiffCl .* vss) ./ vmyo
    du[22] = dcli_dt
    ...
    Gto =
        Ito_Multiplier_1 .*
        ((celltype_val == T(1.0) || celltype_val == T(2)) ? (T(2) * Gto_b) : (Gto_b))
    ...
    ICaK = ICaK_i + ICaK_ss
    dkss_dt = -JdiffK + (Acap .* (-ICaK_ss)) ./ ((F_param .* vss))
    du[55] = dkss_dt
        ...
        Bcai .* (
            -dCa_TRPN_dt .* trpnmax + (
                (
                    ...
                    (((T(2) * F_param) .* vmyo)) - Jup .* vnsr ./ vmyo
                ) + (Jdiff .* vss) ./ vmyo
            )
        )
    du[59] = dcai_dt
    dnai_dt =
        ...
        ) ./ ((F_param .* vmyo)) + (JdiffNa .* vss) ./ vmyo
    du[60] = dnai_dt
    dcass_dt =
        Bcass .* (
            -Jdiff + (
                (Acap .* (-(ICaL_ss - T(2) * INaCa_ss))) ./ (((T(2) * F_param) .* vss)) +
                (Jrel .* vjsr) ./ vss
            )
        )
    du[61] = dcass_dt
    dnass_dt = -JdiffNa + (Acap .* (-(ICaNa_ss + T(3.0) * INaCa_ss))) ./ ((F_param .* vss))
    du[62] = dnass_dt
    dv_dt = -(
        INa_INa +
        INaL_INaL +
