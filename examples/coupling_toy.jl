# Minimal demonstration of CytoZoo's model coupling, solved monolithically (one ODE solver over
# the whole combined system):
...
println(
    "  d(2) = ", round(sol.u[end][state_index(coupled, :d)]; digits = 5),
    "   (owner A: exp(-2) = ", round(exp(-2.0); digits = 5), "; B's du(x)=3 discarded)"
)
println("  y(2) = ", round(sol.u[end][state_index(coupled, :B_y)]; digits = 5))

# --- 2. Connect: a reader integrates A's `d`, read through a parameter slot ------------------
struct Reader <: AbstractCardiacCellModel
    parameters::Vector{Float64}     # slot 1 = :d_ext, written by the connect edge each eval
end
Reader() = Reader([0.0])
...
sol2 = solve(ODEProblem(coupled2, (0.0, 2.0)), Tsit5(); dt = 0.01, adaptive = false)
println(
    "connect → acc(2) = ", round(sol2.u[end][state_index(coupled2, :R_acc)]; digits = 5),
